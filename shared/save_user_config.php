...
switch ($_POST['field']){
	...
	case "data_colloqui":
		...
		break;
	case 'ordine_circolari':
		$string = $_POST['active'];
		try {
			$check_exist = $db->executeCount("SELECT COUNT(*) FROM rb_parametri_utente WHERE id_utente = {$_SESSION['__user__']->getUid()} AND id_parametro = 5");
			if($check_exist){
				// update
				$db->executeUpdate("UPDATE rb_parametri_utente SET valore = '{$string}' WHERE id_utente = {$_SESSION['__user__']->getUid()} AND id_parametro = 5");
			}
			else {
				$db->executeUpdate("INSERT INTO rb_parametri_utente (id_utente, id_parametro, valore) VALUES ({$_SESSION['__user__']->getUid()}, 5, '{$string}')");
			}
		} catch (MySQLException $ex){
			$response['status'] = "kosql";
			$response['message'] = "Si è verificato un errore. Si prega di segnalare il problema al responsabile del software";
