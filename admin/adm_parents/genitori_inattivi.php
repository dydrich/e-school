...
$sel_new_parents = "SELECT rb_utenti.uid, rb_utenti.nome, rb_utenti.cognome, rb_utenti.username FROM rb_utenti, rb_gruppi_utente WHERE rb_utenti.uid = rb_gruppi_utente.uid AND gid = 4 AND rb_utenti.uid NOT IN (SELECT id_genitore FROM rb_genitori_figli) 
					UNION SELECT rb_utenti.uid, rb_utenti.nome, rb_utenti.cognome, rb_utenti.username FROM rb_utenti, rb_genitori_figli, rb_alunni WHERE uid = id_genitore AND rb_genitori_figli.id_alunno = rb_alunni.id_alunno AND rb_alunni.attivo = '0' ORDER BY cognome, nome";
$res_new_parents = $db->execute($sel_new_parents);

$navigation_label = "gestione utenti";
$drawer_label = "Elenco genitori inattivi";
