...
<head>
	...
	<script type="text/javaScript">
		...
		var go = function (par, lab){
			if(par == 2){
				if(!confirm("Sei sicuro di voler cancellare questa aula?"))
					return false;
			}
			$('#_i').val(lab);
			$('#action').val(par);
			var url = "labs_manager.php";
...
<div id="main">
	...
	<div id="left_col">
		<form action="labs_manager.php" method="post" id="site_form" class="popup_form">
			<table class="popup_table">
				<tr class="popup_row header_row">
					<td style="width: 30%"><label for="titolo" class="popup_title">Nome</label></td>
					<td style="width: 70%">
						<input class="form_input" type="text" name="titolo" id="titolo" style="width: 100%" <?php if(isset($lab)) print("value='".utf8_decode($lab->getName())."'"); else print "autofocus" ?> />
					</td>
				</tr>
				<tr class="popup_row">
					<td style="width: 30%"><label for="sede" class="popup_title">Sede</label></td>
					<td style="width: 70%">
						<select name="sede" id="sede" style="width: 100%">
							<?php
							while($row = $res_sedi->fetch_assoc()) {
							?>
							<option value="<?php echo $row['id_sede'] ?>" <?php if(isset($lab) && $row['id_sede'] == $lab->getVenue()) echo "selected" ?>><?php echo $row['nome'] ?></option>
							<?php
							}
							?>
						</select>
