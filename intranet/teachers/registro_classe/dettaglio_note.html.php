...
<div id="main" style="clear: both; ">
	<div style="top: -7px; margin-left: 925px; margin-bottom: -25px" class="rb_button">
...
<tr id="row<?php echo $row['id_nota'] ?>" style="<?php echo $background ?>">
	...
	<td style="width: 45%; text-align: center; "><a id="comlink<?php echo $row['id_nota'] ?>" data-id="<?php echo $row['id_nota'] ?>" data-permission="<?php if ($row['docente'] == $_SESSION['__user__']->getUid()) echo 1; else echo 0 ?>" data-stid="<?php echo $student_id ?>" class="note_link<?php echo $attention ?>" href="#" style=""><?php echo $row['descrizione'] ?></a></td>
</tr>
<?php 
	$index++;
}
...
<!-- tipi nota -->
<div id="tipinota" style="display: none; height: 220px">
    <p style="line-height: 16px"><a style="font-weight: normal" href="dettaglio_note.php?al=<?php echo $student_id ?>&q=<?php echo $q ?>&order=data">Tutte le note</a></p>
<?php
while($t = $res_tipi->fetch_assoc()){
?>
    <p style="line-height: 16px"><a style="font-weight: normal" href="dettaglio_note.php?al=<?php echo $student_id ?>&q=<?php echo $q ?>&order=data&tipo=<?php echo $t['id_tiponota'] ?>"><?php echo $t['descrizione'] ?></a></p>
<?php } ?>
</div>
<!-- tipi nota -->
