...
require_once 'FirstGradeSupportTeacherRecordBookPDF.php';

class FirstGradeSupportTeacherRecordBook extends FirstGradeTeacherRecordBook{
	...
	public function __construct(SchoolUserBean $teacher, MySQLDataLoader $ds, $pt, AnnoScolastico $y, $p, SchoolYear $sy){
		...
		$this->pdf = new FirstGradeSupportTeacherRecordBookPDF(PDF_PAGE_ORIENTATION, PDF_UNIT, PDF_PAGE_FORMAT, true, 'UTF-8', false);
	...
	public function setCls($cls){
		$this->cls = $this->loadClassData($cls);
	}
	...
	public function loadStudentData($cls, $st){
		...
		$sel_comp = "SELECT alunno, voto, quadrimestre FROM rb_scrutini WHERE classe = {$cls} AND anno = {$this->year->get_ID()} AND materia = 2 AND alunno = {$st}";
	...
	public function existsRecordBook($cls, $st){
		$sel_ex = "SELECT id FROM rb_registri_personali WHERE anno = {$this->year->get_ID()} AND docente = {$this->teacher->getUid()} AND classe = {$cls} AND alunno = {$st}";
