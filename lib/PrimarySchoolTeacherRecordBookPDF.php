...
require_once 'PrimarySchoolSupportTeacherRecordBookPDF.php';
require_once 'Encoding.php';

class PrimarySchoolTeacherRecordBookPDF extends TeacherRecordBookPDF{
	...
	public function init($t, $c, $i, $sub, $att){
		...
		$this->SetTitle('Registro del docente di scuola primaria');
	...
	public function sessionGrades($students, $subject){
		...
		$this->Cell(35, 5, "1Q", 'LRTB', 0, 'C', 0, '', 0);
		$this->Cell(35, 5, "2Q", 'LRTB', 0, 'C', 0, '', 0);
		$this->Ln();
		
		foreach ($students as $k => $v){
			$this->SetFont('helvetica', '', '11');
			$this->Cell(91, 5, $v['cognome']." ".$v['nome'], 'LRTB', 0, 'L', 0, '', 0);
			$row = $v;
			if (isset($row['voto1q']) && $row['voto1q'] != ''){
				if ($subject == 30){
			...
			*/
			...
			if (isset($row['comp1q']) && $row['comp1q'] != ''){
				...
				$this->Cell(35, 5, '', 'LRTB', 0, 'C', 0, '', 0);
			}
			$this->Cell(35, 5, $voti_comportamento_primaria[$row['comp2q']]['nome'], 'LRTB', 0, 'C', 0, '', 0);
			$this->Ln();
		}
		//$this->Cell(180, 12, "* Per voto di consiglio", 0, 1, 'L', 0, '', 0, 0, '', 'C');
	}
	
	public function studentGrades($students, $subject){
		$voti_religione = array("4" => "Insufficiente", "6" => "Sufficiente", "8" => "Buono", "9" => "Distinto", "10" => "Ottimo");
	...
	public function lessonAbsences($lessons){
		$this->AddPage("P", "A4");
		$this->_page++;
		$this->setPage($this->_page, true);
		$this->SetFont('helvetica', 'B', '13');
